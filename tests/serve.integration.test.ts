...
describe.skipIf(!HAS_CLAUDE)('serve integration', () => {
  ...
  test('directory traversal via percent-encoded `..` is rejected (issue #5)', async () => {
    ...
    // Every case below uses an extension in the static-asset whitelist
    // (.ts/.js/.css/.json/.svg/.png) so the request actually reaches the
    // new traversal guard rather than falling through to the default 404.
    const cases = [
      `http://127.0.0.1:${PORT}/%2e%2e/package.json`,
      `http://127.0.0.1:${PORT}/%2e%2e/%2e%2e/package.json`,
      `http://127.0.0.1:${PORT}/%2e%2e/README.md.png`,
      `http://127.0.0.1:${PORT}/%2e%2e/SECURITY.md.svg`,
      `http://127.0.0.1:${PORT}/%2e%2e/src/serve.ts`,
    ];
