...
- messages from the browser arrive at turn ends: while someone is attached, a listening window (default 45 s, `ORC_STOP_LINGER_MS`) keeps the session responsive after each turn; a browser message sent while this session sits idle past that window is delivered the next time you prompt here,
