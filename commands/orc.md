...
- messages from the browser arrive at turn ends: while someone is attached, a listening window keeps the session responsive after each turn — 45 s normally (`ORC_STOP_LINGER_MS`), stretching to 30 min per turn while the conversation is being driven from the browser (`ORC_STOP_LINGER_ACTIVE_MS`), so remote back-and-forth flows continuously; a browser message that misses the window gets an immediate "queued — session is idle" note in the browser and is delivered the next time you prompt here (an idle terminal also shows a waiting-message hint),
